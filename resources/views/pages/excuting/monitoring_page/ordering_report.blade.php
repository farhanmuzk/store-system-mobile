<x-app-layout>
    ...
    <div class="px-8 pb-12">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Form Update Teks -->
            <form method="POST" action="{{ route('ordering-report.update') }}" class="bg-gray-800 rounded-xl shadow-md p-6 mt-4 text-white">
                @csrf
                ...
                <select name="report_id" id="report_id" class="w-full mb-4 rounded-md text-black px-4 py-2">
                    @foreach($reports as $report)
                        @if($report->type_report === 'ordering' && empty($report->teks))
